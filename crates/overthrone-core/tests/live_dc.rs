...
impl DcConfig {
    fn require() -> Self {
        Self {
            host: std::env::var("OT_DC_HOST")
                .expect("Set OT_DC_HOST env var"),
            domain: std::env::var("OT_DOMAIN")
                .expect("Set OT_DOMAIN env var"),
            username: std::env::var("OT_USERNAME")
                .expect("Set OT_USERNAME env var"),
            password: std::env::var("OT_PASSWORD")
                .expect("Set OT_PASSWORD env var"),
        }
    }
}
...
async fn graph_bloodhound_export_live() {
    ...
    let parsed: serde_json::Value = serde_json::from_str(&bh_json)
        .expect("BloodHound output is not valid JSON");
    assert!(parsed.get("nodes").is_some() || parsed.get("data").is_some(),
