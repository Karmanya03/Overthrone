...
impl SmbSession {
    ...
    /// Stub — directory listing requires SMB2 QUERY_DIRECTORY which
    /// the `smb` crate doesn't expose on `Directory` directly yet.
    /// List files and subdirectories in a remote SMB share path.
/// List files in a remote SMB share directory.
///
/// The `smb` crate v0.11 does not expose SMB2 QUERY_DIRECTORY,
/// so we shell out to `smbclient` (from Samba) as a fallback.
/// Falls back to a clear error if smbclient is not installed.
pub async fn list_directory(
    ...
    // Normalize path for smbclient (forward slashes, no leading slash)
    ...
    // Shell out to smbclient for directory listing
    let output = tokio::process::Command::new("smbclient")
        ...
        .map_err(|e| {
            OverthroneError::Smb(format!(
                "smbclient not found or failed to execute: {e}. \
                 Install samba-client or upgrade the smb crate to a version \
                 that supports QUERY_DIRECTORY."
            ))
        })?;
    ...
    if !output.status.success() {
        ...
        return Err(OverthroneError::Smb(format!(
            "smbclient listing failed for '\\\\{}\\{}\\{}': {}",
            self.target, share, remote_path, stderr.trim()
        )));
    }
    ...
    // Parse smbclient `ls` output lines like:
    //   .                                   D        0  Wed Feb 18 12:00:00 2026
    //   ..                                  D        0  Wed Feb 18 12:00:00 2026
    //   payload.exe                         A    45056  Wed Feb 18 12:00:00 2026
    for line in stdout.lines() {
        ...
        // Split into: name, attributes, size, date
    ...
    pub async fn read_file(
        ...
        file.close().await.map_err(|e| {
            OverthroneError::Smb(format!("Close failed: {e}"))
        })?;
    ...
    // ═══════════════════════════════════════════════════════
    //  Named Pipe Operations (via SMB2 IOCTL/TRANSACT)
    // ═══════════════════════════════════════════════════════

    /// Transact on a named pipe: open pipe, use FSCTL_PIPE_TRANSCEIVE via ioctl,
    /// then close. This is the correct SMB2 way to do DCE/RPC over named pipes.
    pub async fn pipe_transact(
        &self,
        pipe_name: &str,
        request: &[u8],
    ) -> Result<Vec<u8>> {
        ...
        // Use FSCTL_PIPE_TRANSCEIVE (0x0011C017) via ioctl for pipe I/O.
        // This is the standard SMB2 mechanism for named pipe transactions.
        let response = pipe
        ...
        pipe.close().await.map_err(|e| {
            OverthroneError::Smb(format!("Pipe close failed: {e}"))
        })?;
