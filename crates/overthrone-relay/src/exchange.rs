...
async fn handle_exchange_relay(
    ...
    let negotiate_payload = build_exchange_http_request(&config.mapi_path, &ntlm_negotiate);
    exchange_stream
    ...
    let authenticate_payload = build_exchange_http_request(&config.mapi_path, &ntlm_authenticate);
    exchange_stream
...
fn build_exchange_http_request(path: &str, ntlm_blob: &[u8]) -> Vec<u8> {
...
mod tests {
    ...
    fn test_build_http_request() {
        ...
        let req = build_exchange_http_request("/mapi/", ntlm);
