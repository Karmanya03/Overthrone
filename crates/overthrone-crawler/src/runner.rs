...
use crate::{escalation, foreign, interrealm, mssql_links, pam, sid_filter, trust_map};
...
pub struct CrawlerResult {
    ...
    pub interrealm_attacks: Vec<interrealm::InterRealmAttack>,
...
pub async fn run_crawler(
    ...
    let graph_summary = format!("{} domains, {} trusts", graph.domains.len(), graph.trusts.len());
    pb.println(format!("  {} {} → {}",
        "✓".green().bold(), "trust_map".bright_white(), graph_summary.as_str().bright_green()
    ));
    pb.inc(1);

    // ── 2. Foreign memberships ──
    pb.set_message("foreign...");
    let foreign_memberships = foreign::analyze_foreign_memberships(
        &config.domain, &reaper_data.groups,
    );
    let fm_count = foreign_memberships.len().to_string();
    pb.println(format!("  {} {} → {} found",
        "✓".green().bold(), "foreign".bright_white(), fm_count.as_str().bright_green()
    ));
    pb.inc(1);

    // ── 3. Escalation paths ──
    pb.set_message("escalation...");
    let escalation_paths = escalation::find_escalation_paths(
        &graph, &foreign_memberships, reaper_data,
    );
    let esc_count = escalation_paths.len().to_string();
    pb.println(format!("  {} {} → {} paths",
        "✓".green().bold(), "escalation".bright_white(), esc_count.as_str().bright_green()
    ));
    pb.inc(1);

    // ── 4. Inter-realm attacks ──
    pb.set_message("interrealm...");
    let interrealm_attacks = interrealm::find_interrealm_attacks(&config.domain, &graph);
    let ir_count = interrealm_attacks.len().to_string();
    pb.println(format!("  {} {} → {} attacks",
        "✓".green().bold(), "interrealm".bright_white(), ir_count.as_str().bright_green()
    ));
    pb.inc(1);
    ...
    pb.println(format!("  {} {} → {} findings",
        "✓".green().bold(), "sid_filter".bright_white(), sf_count.as_str().bright_green()
    ));
    pb.inc(1);

    // ── 6. MSSQL link chains ──
    pb.set_message("mssql_links...");
    let mssql_chains = mssql_links::build_mssql_chains(
        &config.domain, &reaper_data.mssql_instances,
    );
    let ms_count = mssql_chains.len().to_string();
    pb.println(format!("  {} {} → {} chains",
        "✓".green().bold(), "mssql_links".bright_white(), ms_count.as_str().bright_green()
    ));
    pb.inc(1);
    ...
    pb.println(format!("  {} {} → {} findings",
        "✓".green().bold(), "pam".bright_white(), pam_count.as_str().bright_green()
    ));
    pb.inc(1);
    ...
    let total_findings = foreign_memberships.len()
        ...
        + interrealm_attacks.len()
        + sid_filter_findings.len()
        + mssql_chains.len()
        + pam_findings.len();
    let total_str = total_findings.to_string();

    println!("\n{}", "─── Crawler Summary ───".bright_cyan());
    println!("  Trust graph: {} domains, {} trusts",
