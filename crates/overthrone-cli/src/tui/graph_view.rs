...
fn edge_abuse_info(edge_type: &EdgeType) -> Option<&'static str> {
 ...
 EdgeType::AllExtendedRights => Some("Covers all extended rights -- check individual RSOP"),
 ...
 EdgeType::AddSelf => {
 Some("Self-write validated right -- add your own account to the group")
 }
 EdgeType::CreateChild => Some("CreateChild -- create new AD objects in the container"),
 EdgeType::WriteSelf => Some("WriteSelf -- modify attributes on self/group membership"),
 ...
 EdgeType::CanRDP => {
 Some("xfreerdp / mstsc -- GUI access; local admin may not be required")
 }
...
pub fn render_graph(f: &mut Frame, area: Rect, app: &App) {
 ...
 warn!("Mutex poisoned in GraphView -- recovering data");
 e.into_inner()
 });
